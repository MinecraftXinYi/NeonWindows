...
using NeonWindows.UI.Input.Core;

namespace NeonWindows.UI.Windowing;

public unsafe class Window : Win32WindowBase, IDisposable
{
    public override nint Handle { get; }

    private static readonly string wndClassName = WndClassManager.RegisterWndClassForType<Window>(WndClassStyleInfo.Default, WndProcManager.WndProcPtr);
    ...
    public Window(in WindowSettings settings)
    {
        ...
        if (Handle == default) Marshal.ThrowExceptionForHR(Marshal.GetHRForLastWin32Error());
        WndProcManager.AddProcDelegate(Handle, WndProcInstanceEntry);
    ...
    public static void Run()
    {
        ...
        {
            Thread.Sleep(1);
            if (PInvoke.PeekMessage(out msg, default, 0, 0, PM_REMOVE))
            {
    ...
    public void Close()
    {
        if(!_ClosedWin)
        {
            ...
            WndClassManager.UnregisterWndClass(wndClassName);
            PInvoke.DestroyWindow((HWND)Handle);
            WndProcManager.RemoveProcDelegate(Handle);
    ...
    #region WndProc
    private nint WndProcInstanceEntry(nint hWnd, uint message, nuint wParam, nint lParam)
    {
        if (hWnd != Handle) return PInvoke.DefWindowProc((HWND)hWnd, message, wParam, lParam);
        return WndProc(hWnd, message, wParam, lParam);
    }

    protected nint WndProc(nint hWnd, uint message, nuint wParam, nint lParam)
    {
        switch (message)
        {
            #region Windowing
            ...
            case PInvoke.WM_SIZE:
                {
                    ...
                    {
                        this.OnResize(new ResizeEventArgs(new(MinWinDef.LOWORD((uint)lParam), MinWinDef.HIWORD((uint)lParam))));
                    }
                }
                break;
            ...
            case PInvoke.WM_XBUTTONDOWN:
                switch (KeyInputInfo.GET_XBUTTON_WPARAM(wParam))
                {
    ...
    private void AdjustDimensionsWin()
    {
        switch (State)
        {
            case WindowState.Normal:
                ...
                PInvoke.SetWindowLong((HWND)Handle, WINDOW_LONG_PTR_INDEX.GWL_STYLE, (int)NorStyle);
                PInvoke.SetWindowPos((HWND)Handle, default, 0, 0, 0, 0,
            ...
            case WindowState.Minimized:
                ...
                PInvoke.SetWindowLong((HWND)Handle, WINDOW_LONG_PTR_INDEX.GWL_STYLE, (int)MinStyle);
                PInvoke.SetWindowPos((HWND)Handle, default, 0, 0, 0, 0,
            ...
            case WindowState.Maximixed:
                ...
                PInvoke.SetWindowLong((HWND)Handle, WINDOW_LONG_PTR_INDEX.GWL_STYLE, (int)MaxStyle);
                PInvoke.SetWindowPos((HWND)Handle, HWND.HWND_TOP,
            ...
            case WindowState.FullScreen:
                ...
                PInvoke.SetWindowLong((HWND)Handle, WINDOW_LONG_PTR_INDEX.GWL_STYLE, (int)FullScreenStyle);
                PInvoke.SetWindowPos((HWND)Handle, HWND.HWND_TOP,
                    _MONITORINFO1.rcMonitor.left, _MONITORINFO1.rcMonitor.top,
                    _MONITORINFO1.rcMonitor.right - _MONITORINFO1.rcMonitor.left,
                    _MONITORINFO1.rcMonitor.bottom - _MONITORINFO1.rcMonitor.top,
