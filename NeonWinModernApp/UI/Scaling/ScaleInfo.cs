...
public static class ScaleInfo
{
    ...
    public static uint SystemDpi
        => XDpiInfoApi.GetDpiForSystem();

    /// <summary>
    /// 系统 DPI 值 (忽略 DPI 感知模式)。
    /// </summary>
    public static uint SystemDpiDirect
        => XDpiInfoApi.GetDpiFromDpiAwarenessContext(DPI_AWARENESS_CONTEXT.DPI_AWARENESS_CONTEXT_SYSTEM_AWARE);
    ...
    public static uint WindowDpi(nint hWnd)
        => XDpiInfoApi.GetDpiForWindow(hWnd);
    ...
    public static float DpiToScaleFactor(uint dpi)
        => (float)dpi / XDpiInfoApi.USER_DEFAULT_SCREEN_DPI;
}
